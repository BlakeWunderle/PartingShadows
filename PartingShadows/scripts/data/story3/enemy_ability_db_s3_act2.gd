...
static func web_snare() -> AbilityData:
	return _make("Web Snare", "Sticky threads tangle around the ankles.", Enums.StatType.SPEED, 6, 2, true, 2, false, 0, 0.0)
...
static func gloom_web() -> AbilityData:
	return _make("Gloom Web", "A net of darkness settles over everything.", Enums.StatType.DEFENSE, 10, 2, true, 4, true, 0, 0.0)
...
static func swift_current() -> AbilityData:
	return _make("Swift Current", "The beast rides the flow and accelerates.", Enums.StatType.SPEED, 6, 2, false, 3, false, 0, 0.0)
...
static func gilded_veil() -> AbilityData:
	return _make("Gilded Veil", "The wight reaches through its frame, draping an ally in spectral canvas.", Enums.StatType.DEFENSE, 10, 2, false, 3, false, 0, 0.0)
...
static func shatter_ward() -> AbilityData:
	return _make("Shatter Ward", "The shade peels away defenses like old paint.", Enums.StatType.DEFENSE, 10, 2, true, 3, false, 0, 0.0)
...
static func shadow_shroud() -> AbilityData:
	return _make("Shadow Shroud", "The pursuer melts into its own darkness, blows glancing off the shifting form.", Enums.StatType.DEFENSE, 12, 2, false, 3, false, 0, 0.0)
...
static func fading_grasp() -> AbilityData:
	return _make("Fading Grasp", "Shadow fingers close around the fastest target, dragging their reflexes into darkness.", Enums.StatType.SPEED, 8, 2, true, 3, false, 0, 0.0)
...
static func constrict() -> AbilityData:
	return _make("Constrict", "Tendrils wind around every target at once, squeezing the speed from their limbs.", Enums.StatType.SPEED, 8, 2, true, 4, true, 0, 0.0)
...
static func fade() -> AbilityData:
	return _make("Fade", "Everything feels slower, heavier, further away.", Enums.StatType.SPEED, 8, 2, true, 4, true, 0, 0.0)
...
static func merchant_guard() -> AbilityData:
	return _make("Merchant Guard", "The shopkeeper spots a gap in the opponent's guard.", Enums.StatType.DEFENSE, 10, 2, true, 3, false, 0, 0.0)
...
static func tainted_salve() -> AbilityData:
	return _make("Tainted Salve", "What was meant to heal now corrodes.", Enums.StatType.MAGIC_ATTACK, 0, 3, true, 3, false, 3, 0.0)
...
static func tether_pull() -> AbilityData:
	return _make("Tether Pull", "The threads jerk the target off balance.", Enums.StatType.SPEED, 6, 2, true, 3, false, 0, 0.0)
...
static func reinforced_threads() -> AbilityData:
	return _make("Reinforced Threads", "Threads unwind the target's protections with mechanical precision.", Enums.StatType.DEFENSE, 10, 2, true, 3, false, 0, 0.0)
