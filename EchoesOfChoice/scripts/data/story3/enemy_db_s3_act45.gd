...
static func create_cult_acolyte(n: String, lvl: int = 15) -> FighterData:
	...
	f.health = EH.es(529, 608, 5, 8, lvl, 15); f.max_health = f.health
	f.mana = EH.es(19, 23, 1, 2, lvl, 15); f.max_mana = f.mana
	f.physical_attack = EH.es(10, 14, 0, 2, lvl, 15)
	f.physical_defense = EH.es(36, 45, 2, 3, lvl, 15)
	f.magic_attack = EH.es(98, 113, 3, 5, lvl, 15)
	f.magic_defense = EH.es(45, 54, 2, 4, lvl, 15)
	f.speed = EH.es(36, 43, 2, 3, lvl, 15)
	f.crit_chance = 15; f.crit_damage = 3; f.dodge_chance = 31
...
static func create_cult_enforcer(n: String, lvl: int = 15) -> FighterData:
	...
	f.health = EH.es(630, 721, 6, 9, lvl, 15); f.max_health = f.health
...
static func create_cult_hexer(n: String, lvl: int = 15) -> FighterData:
	...
	f.health = EH.es(450, 520, 4, 7, lvl, 15); f.max_health = f.health
	f.mana = EH.es(19, 23, 1, 2, lvl, 15); f.max_mana = f.mana
	f.physical_attack = EH.es(10, 14, 0, 2, lvl, 15)
	f.physical_defense = EH.es(30, 38, 1, 3, lvl, 15)
	f.magic_attack = EH.es(100, 115, 3, 5, lvl, 15)
	f.magic_defense = EH.es(43, 52, 2, 4, lvl, 15)
	f.speed = EH.es(39, 45, 2, 3, lvl, 15)
	f.crit_chance = 16; f.crit_damage = 3; f.dodge_chance = 35
...
static func create_thread_guard(n: String, lvl: int = 16) -> FighterData:
	...
	f.health = EH.es(600, 685, 6, 9, lvl, 16); f.max_health = f.health
	f.mana = EH.es(8, 11, 1, 1, lvl, 16); f.max_mana = f.mana
	f.physical_attack = EH.es(103, 118, 3, 5, lvl, 16)
	f.physical_defense = EH.es(52, 61, 2, 4, lvl, 16)
	f.magic_attack = EH.es(8, 12, 0, 1, lvl, 16)
	f.magic_defense = EH.es(42, 50, 2, 3, lvl, 16)
...
static func create_dream_hound(n: String, lvl: int = 16) -> FighterData:
	...
	f.health = EH.es(430, 495, 5, 8, lvl, 16); f.max_health = f.health
	f.mana = EH.es(8, 11, 1, 1, lvl, 16); f.max_mana = f.mana
	f.physical_attack = EH.es(91, 105, 3, 5, lvl, 16)
	f.physical_defense = EH.es(36, 43, 2, 3, lvl, 16)
	f.magic_attack = EH.es(8, 12, 0, 1, lvl, 16)
	f.magic_defense = EH.es(34, 41, 2, 3, lvl, 16)
...
static func create_cult_ritualist(n: String, lvl: int = 17) -> FighterData:
	...
	f.physical_defense = EH.es(42, 51, 2, 4, lvl, 17)
...
static func create_high_weaver(n: String, lvl: int = 17) -> FighterData:
	...
	f.health = EH.es(501, 577, 5, 8, lvl, 17); f.max_health = f.health
	f.mana = EH.es(22, 25, 2, 3, lvl, 17); f.max_mana = f.mana
	f.physical_attack = EH.es(10, 14, 0, 2, lvl, 17)
	f.physical_defense = EH.es(36, 44, 2, 3, lvl, 17)
	f.magic_attack = EH.es(112, 129, 3, 6, lvl, 17)
	f.magic_defense = EH.es(51, 59, 2, 4, lvl, 17)
...
static func create_shadow_fragment(n: String, lvl: int = 18) -> FighterData:
	...
	f.crit_chance = 18; f.crit_damage = 3; f.dodge_chance = 26
...
static func create_the_threadmaster(n: String, lvl: int = 18) -> FighterData:
	...
	f.physical_attack = EH.es(93, 107, 3, 5, lvl, 18)
