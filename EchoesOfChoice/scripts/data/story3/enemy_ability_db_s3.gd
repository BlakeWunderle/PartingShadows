...
static func shimmer_bolt() -> AbilityData:
	return _make("Shimmer Bolt", "A flickering mote of light that stings on contact.", Enums.StatType.MAGIC_ATTACK, 6, 0, true, 2, false, 0, 0.0, 1)

static func daze() -> AbilityData:
	return _make("Daze", "The light pulses, leaving the mind sluggish.", Enums.StatType.SPEED, 5, 2, true, 2, false, 0, 0.0, 2)

# --- Phantasm ---

static func phase_strike() -> AbilityData:
	return _make("Phase Strike", "A translucent fist passes through armor and flesh alike.", Enums.StatType.MIXED_ATTACK, 7, 0, true, 3, false, 0, 0.0, 1)

static func unnerve() -> AbilityData:
	return _make("Unnerve", "The air grows cold and confidence falters.", Enums.StatType.DEFENSE, 5, 2, true, 2, false, 0, 0.0, 2)

# --- Shade Moth ---

static func dust_wing() -> AbilityData:
	return _make("Dust Wing", "Scales of shadow scrape across exposed skin.", Enums.StatType.PHYSICAL_ATTACK, 6, 0, true, 2, false, 0, 0.0, 1)

static func flit() -> AbilityData:
	return _make("Flit", "The moth darts erratically, impossible to track.", Enums.StatType.SPEED, 7, 2, false, 2, false, 0, 0.0, 2)

# --- Slumber Beast ---

static func heavy_paw() -> AbilityData:
	return _make("Heavy Paw", "A massive paw crashes down with dreamlike weight.", Enums.StatType.PHYSICAL_ATTACK, 9, 0, true, 2, false, 0, 0.0, 1)

static func drowsy_roar() -> AbilityData:
	return _make("Drowsy Roar", "A deep rumble that makes eyelids heavy.", Enums.StatType.SPEED, 5, 2, true, 4, true, 0, 0.0, 3)

# --- Fog Wraith ---

static func mist_tendril() -> AbilityData:
	return _make("Mist Tendril", "A tendril of fog wraps tight and squeezes.", Enums.StatType.MAGIC_ATTACK, 7, 0, true, 3, false, 0, 0.0, 1)

static func chill_fog() -> AbilityData:
	return _make("Chill Fog", "Cold mist settles over everyone.", Enums.StatType.MAGIC_ATTACK, 6, 0, true, 4, true, 0, 0.0, 3)

# --- Sleep Stalker ---

static func dream_fang() -> AbilityData:
	return _make("Dream Fang", "Teeth that feel real enough to draw blood.", Enums.StatType.PHYSICAL_ATTACK, 9, 0, true, 2, false, 0, 0.0, 1)

static func shadow_lunge() -> AbilityData:
	return _make("Shadow Lunge", "A blur of darkness closes the distance in an instant.", Enums.StatType.PHYSICAL_ATTACK, 12, 0, true, 3, false, 0, 0.0, 2)

# --- Mirror Shade ---

static func reflected_strike() -> AbilityData:
	return _make("Reflected Strike", "Your own movements turned against you.", Enums.StatType.MIXED_ATTACK, 7, 0, true, 3, false, 0, 0.0, 1)

static func mimic_stance() -> AbilityData:
	return _make("Mimic Stance", "The shade copies a defensive posture it observed.", Enums.StatType.DEFENSE, 5, 2, false, 2, false, 0, 0.0, 2)

# --- Thorn Dreamer ---

static func briar_lash() -> AbilityData:
	return _make("Briar Lash", "Thorny vines lash out and keep cutting.", Enums.StatType.PHYSICAL_ATTACK, 6, 3, true, 3, false, 2, 0.0, 2)

static func spore_cloud() -> AbilityData:
	return _make("Spore Cloud", "A cloud of numbing spores drifts over the field.", Enums.StatType.DEFENSE, 5, 2, true, 4, true, 0, 0.0, 3)
...
static func savage_bite() -> AbilityData:
	return _make("Savage Bite", "Jaws snap with force that shouldn't exist in a dream.", Enums.StatType.PHYSICAL_ATTACK, 10, 0, true, 3, false, 0, 0.0, 1)

static func howl() -> AbilityData:
	return _make("Howl", "The pack surges forward at the sound.", Enums.StatType.SPEED, 5, 2, false, 3, true, 0, 0.0, 3)

# --- Dream Weaver ---

static func thread_bolt() -> AbilityData:
	return _make("Thread Bolt", "A needle of woven light pierces through.", Enums.StatType.MAGIC_ATTACK, 10, 0, true, 3, false, 0, 0.0, 1)

static func woven_ward() -> AbilityData:
	return _make("Woven Ward", "Threads of dream-stuff harden into a barrier.", Enums.StatType.DEFENSE, 6, 2, false, 3, true, 0, 0.0, 3)

# --- Somnolent Serpent ---

static func venom_coil() -> AbilityData:
	return _make("Venom Coil", "Fangs sink in and the poison keeps working.", Enums.StatType.PHYSICAL_ATTACK, 9, 3, true, 3, false, 3, 0.0, 2)

static func sleep_fang() -> AbilityData:
	return _make("Sleep Fang", "A bite that numbs the limbs.", Enums.StatType.SPEED, 6, 2, true, 3, false, 0, 0.0, 2)

# --- Twilight Stalker ---

static func dusk_blade() -> AbilityData:
	return _make("Dusk Blade", "A blade forged from the edge of waking.", Enums.StatType.PHYSICAL_ATTACK, 12, 0, true, 3, false, 0, 0.0, 1)

static func vanish_strike() -> AbilityData:
	return _make("Vanish Strike", "It disappears and reappears behind you.", Enums.StatType.PHYSICAL_ATTACK, 15, 0, true, 4, false, 0, 0.0, 2)
...
static func shield_bash() -> AbilityData:
	return _make("Shield Bash", "A wall of shadow slams forward.", Enums.StatType.PHYSICAL_ATTACK, 10, 0, true, 3, false, 0, 0.0, 1)

# --- Hollow Echo ---

static func echo_drain() -> AbilityData:
	return _make("Echo Drain", "It feeds on the reverberations of your voice.", Enums.StatType.MAGIC_ATTACK, 9, 0, true, 3, false, 0, 0.3, 2)

static func dissonance() -> AbilityData:
	return _make("Dissonance", "Conflicting sounds shatter concentration.", Enums.StatType.DEFENSE, 6, 2, true, 3, false, 0, 0.0, 2)

# --- Waking Terror ---

static func scream_blast() -> AbilityData:
	return _make("Scream", "A sound that shouldn't be possible tears through the air.", Enums.StatType.MAGIC_ATTACK, 9, 0, true, 4, true, 0, 0.0, 3)

static func terror_wave() -> AbilityData:
	return _make("Terror Wave", "A pulse of raw fear washes over everyone.", Enums.StatType.SPEED, 6, 2, true, 4, true, 0, 0.0, 3)

# --- Clock Specter ---

static func time_rend() -> AbilityData:
	return _make("Time Rend", "The hands of the clock spin backward through flesh.", Enums.StatType.MIXED_ATTACK, 10, 0, true, 3, false, 0, 0.0, 1)
...
static func nightmare_crush() -> AbilityData:
	return _make("Nightmare Crush", "The full weight of dread made physical.", Enums.StatType.PHYSICAL_ATTACK, 13, 0, true, 4, false, 0, 0.0, 2)

static func dream_rend() -> AbilityData:
	return _make("Dream Rend", "The dreamscape tears apart around you.", Enums.StatType.MAGIC_ATTACK, 12, 0, true, 5, true, 0, 0.0, 3)

static func dread_aura() -> AbilityData:
	return _make("Dread Aura", "Courage withers in its presence.", Enums.StatType.DEFENSE, 6, 2, true, 4, true, 0, 0.0, 3)
...
static func mind_spike() -> AbilityData:
	return _make("Mind Spike", "A focused thought driven like a nail.", Enums.StatType.MAGIC_ATTACK, 12, 0, true, 3, false, 0, 0.0, 1)

static func phase_shift() -> AbilityData:
	return _make("Phase Shift", "The phantom flickers between states of being.", Enums.StatType.DODGE_CHANCE, 15, 2, false, 3, false, 0, 0.0, 2)

# --- Thread Spinner ---

static func woven_mend() -> AbilityData:
	return _make("Woven Mend", "Threads stitch together what was broken.", Enums.StatType.DEFENSE, 8, 2, false, 4, true, 0, 0.0, 3)
