...
static func dazzle() -> AbilityData:
	return _make("Dazzle", "A burst of blinding light leaves the target swinging at shadows.", Enums.StatType.ATTACK, 3, 2, true, 3, false, 0, 0.0, 2)
...
static func phantasmal_haze() -> AbilityData:
	return _make("Phantasmal Haze", "Hallucinogenic spores cloud the senses, making every target seem to multiply.", Enums.StatType.ATTACK, 4, 2, true, 4, true, 0, 0.0, 3)
...
static func piercing_gaze_strike() -> AbilityData:
	return _make("Piercing Gaze", "Eyes that cut deeper than any blade.", Enums.StatType.MIXED_ATTACK, 8, 0, true, 3, false, 0, 0.0, 2)
